...
router.get("/", async (req, res) => {
  try {
    ...
    posts
      ...
      : res.status(404).send("No posts are available.");
  } catch (e) {
...
router.post("/", async (req, res) => {
  ...
  try {
    !checkRequiredKeys(req.body, requiredKeys) &&
      res.status(400).send("Post needs both title and content");
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    removedAmount
      ? res.status(201).json(removedAmount)
...
router.put("/:id", async (req, res) => {
  ...
  try {
    !checkAllowedKeys(req.body, allowedKeys) &&
      res.status(400).json("Post edit needs either contents or title");
